...
now = tonumber(now[1] .. '.' .. string.sub(now[2], 1, 3))
...
for i, limit in ipairs(limits) do
    ...
    for j, key in ipairs(KEYS) do
        ...
        -- check our limits
        if tonumber(cur or '0') > limit[2] then
            return '1'
        end
    end
end
...
for i, limit in ipairs(limits) do
    ...
    for j, key in ipairs(KEYS) do
        ...
        redis.call('HINCRBY', key, saved.count_key, 1)
        redis.call('HINCRBY', key, saved.count_key .. saved.block_id, 1)
    end
end
